...
def load_checkpoint(path, device):
    ...
    return generator, predictor, corruption_predictor, cfg


@torch.no_grad()
def jepa_generate(generator, predictor, corruption_predictor, prompt_ids, max_new_tokens, device):
    cfg = generator.cfg
    ...
    for _ in range(max_new_tokens):
        ...
        # Step 4: pick byte where corruption predictor sees least corruption
        # candidate_hiddens = "gen" side, predicted_next = "target" side
        scores = corruption_predictor(candidate_hiddens, predicted_next.expand(256, -1)).squeeze(-1)  # [256]
...
def main():
    ...
    generator, predictor, corruption_predictor, cfg = load_checkpoint(ckpt_path, device)
    print(f"Generating {args.max_tokens} tokens on {device}...")

    prompt_ids = tokenizer.encode(args.prompt)
    output_ids = jepa_generate(generator, predictor, corruption_predictor, prompt_ids, args.max_tokens, device)
